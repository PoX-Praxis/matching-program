...
## 2026-07-09｜実験フェーズクローズ｜決定

外部レビュー完了・事実誤認3件修正済み・決定維持（**nomic-emb-v2・フル768次元・暫定チャンピオン方式**）。
詳細は `eval/analysis/final_report.md` §8。スケール戦略の発火順は `HANDOVER.md` §5 に記録。

---

## 2026-07-08｜外部レビュー完了・決定維持・実験フェーズクローズ｜決定

**外部レビューを受けた文書修正（コード不変・final_report.md）:**
- 事実誤認3件を修正：①「有意差なし」→「本装置では検出不能（n=1・検定未実施）」／①「平均最高・取りこぼし最少」の二重計上→独立信号1勝1敗（4の捕捉数=Qwen3/順位一致度=決勝でNomic）／③「Nomic最小劣化」を根拠から除外→**測定失敗の疑い**（固定パラメータのフル次元前提）・装置修正は将来課題。
- 既知の限界を明記：seeker=設計者1名・必要像一本・評価はTOP30範囲制限・真の偽陰性は測定不能。prefix非対称打ち分けは実機確認済みを§0に追記。
- レビュー側の誤り1件を記録：決勝の順位一致度は Nomic(+0.273)>Qwen3(+0.205)。
- 改善バックログ：低順位負コントロール／言語別分解／コンテキスト長確認／H-3リコール定義化／複数seeker／**律速軸退化の検証=本番最優先**。

**決定は維持：採用モデル `nomic-emb-v2`（決定者 KH）。決め手は②-2ライセンス（EmbGemma脱落）＋供給/監査/コストでNomic優位。①③は決定根拠にしない（①検出不能・③測定失敗の疑い）。**

**実験フェーズをクローズ。** 本番は Nomic・フル768次元で設計。次トラック＝改善バックログ（律速軸退化の検証が最優先）／母数立ち上げ。
