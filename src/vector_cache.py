...
# 本モジュールは src/ に配置。同ディレクトリの embedding_service を import できるようにする。
...
# キャッシュ本体の既定置き場（repo/eval/cache）。eval ランナーと同じ場所を指す（gitignore 配下）。
CACHE_DIR = pathlib.Path(__file__).parent.parent / "eval" / "cache"
